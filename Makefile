...
CARGO_CLIPPY = $(CARGO) clippy --all-features --all-targets --frozen --no-default-features --workspace --  \
	--deny clippy::all \
	--deny clippy::cargo \
	--deny clippy::nursery \
...
CURL = curl -sSLo
CUT = cut
FIND_RM = find . -exec rm -fr {} + -type f -name
GIT = git
GRCOV = grcov
GREP = grep
...
.PHONY: add-helm
add-helm: ## Add the helm
	@$(OS_PACKAGE_MANAGER) install --quiet helm

.PHONY: add-hunspell
add-hunspell: add-hunspell-dictionary ## Add the hunspell
	@$(OS_PACKAGE_MANAGER) install --quiet hunspell

.PHONY: add-hunspell-dictionary
add-hunspell-dictionary: ## Add the hunspell dictionary
	@if [ ! -f "$(HUNSPELL_DICTIONARY).dic" ]; then\
		$(CURL) $(HUNSPELL_DICTIONARY).aff https://cgit.freedesktop.org/libreoffice/dictionaries/plain/en/$(HUNSPELL_DICTIONARY).aff?id=a4473e06b56bfe35187e302754f6baaa8d75e54f;\
		$(CURL) $(HUNSPELL_DICTIONARY).dic https://cgit.freedesktop.org/libreoffice/dictionaries/plain/en/$(HUNSPELL_DICTIONARY).dic?id=a4473e06b56bfe35187e302754f6baaa8d75e54f;\
	fi

.PHONY: add-kubernetes-cli
add-kubernetes-cli: ## Add the kubernetes cli
	@$(OS_PACKAGE_MANAGER) install --quiet kubernetes-cli
